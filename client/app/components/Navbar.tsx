...
function Navbar() {
  ...
  return (
    <div className={`fixed top-0 left-0 z-50 w-full transition-all duration-300 bg-orange-300 ${scrolled ? 'bg-gray-600/50 backdrop-blur-lg' : 'bg-orange-300'}`}>
            <div className='px-5 md:px-14 sm:px-10'>
                <div className="flex items-center justify-between py-4 m-2 text-sm ">
                    {/* App Logo---------------------------------- */}
                    <div>
                        <p onClick={() => router.push('/')} className="text-2xl font-bold text-white cursor-pointer">Green Cabana</p>
                    </div>
                    <ul className="items-start hidden gap-5 font-medium md:flex">
                        ...
                        <div>
                            <p onClick={() => router.push('/booking')} className="px-3 py-2 text-white transition duration-300 ease-in-out delay-150 bg-green-900 rounded-full cursor-pointer hover:text-secondary hover:-translate-y-1 hover:scale-110 hover:text-secondary-50">Book Now</p>
                        </div>
                    </ul>
                    {/*Menu Icon ----------------------------------------*/}
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} lg:hidden right-0 top-0 bottom-0 overflow-hidden  transition-all fixed inset-0 z-40 bg-background/98 backdrop-blur-xl pt-20 `} style={{ opacity: '1', transform: 'none', transition: 'opacity 300ms ease-in-out, transform 300ms ease-in-out' }}>
                        <div className="flex items-center justify-between px-7 ">
                            {/* App Logo-----------------------------------------------------*/}
                            <div>
                                <p onClick={() => router.push('/')} className="text-lg font-bold text-green-600 cursor-pointer">Green Cabana</p>
                            </div>
                            {/* Close Icon--------------------------------------------------*/}
